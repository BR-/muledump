    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            if ( this.loginOnly === false && !this.opt('shrink') && this.opt('hpmp')) {
                var quickslotsArr = c.EquipQS.split(',')

                var quickslots = $('<div class="w100">');
                if (quickslotsArr == "None") // for old mules
				{
					quickslotsArr = ["-1|0", "-1|0"]
					if (c.Has3Quickslots == 1)
					{
						quickslotsArr = ["-1|0", "-1|0", "-1|0"]
					}
				}
                for (var qsIndex = 0; qsIndex < quickslotsArr.length; qsIndex++) {
                    const quickslot = quickslotsArr[qsIndex].split('|');
                    const itemId = quickslot[0];
                    const amount = quickslot[1];

                    if ((c.Has3Quickslots == 1 && qsIndex == 2 && itemId == "-1") || qsIndex != 2 && itemId == "-1" ) {
                        quickslots.append($('<div class="item" style="border: solid 1px #000; align-self:center; flex-shrink:0;"><div>0</div></div>'));
                    }
                    else if (itemId != "-1" && window.items[itemId]) {
                        quickslots.append($('<div class="item" style="background-position: -' + window.items[itemId][3] + 'px -'  + window.items[itemId][4] + 'px; border: solid 1px #000; align-self:center; flex-shrink:0;"><div>'+ amount +'</div></div>'));
                    }
                }
                $c.append(quickslots);
            }
